...
class WakeWordDetector(
    ...
    private fun runPipelineStep(numSteps: Int) {
        Log.d("WakeWordDetector", "Running pipeline step")
    ...
    private fun evaluateWakeWord() {
        ...
        tensor.use {
            classifierSession.run(Collections.singletonMap(classifierInputName, it)).use { res ->
                ...
                if (minVerifierScore > 0 && verifierFile != null) {
                    verifierSession.run(Collections.singletonMap(verifierInputName, it)).use { vRes ->
                        @Suppress("UNCHECKED_CAST")
                        val verifierScore = ((vRes[1].value as List<*>)[0] as OnnxMap).value[1L] as Float
                        Log.d("WakeWordDetector", "Verifier score: $verifierScore")
                        if (verifierScore >= minVerifierScore) onWakeWordDetected(score)
                    }
                } else {
    ...
    class AudioRingBuffer(private val capacity: Int) {
        ...
        fun getLastSamplesInto(dest: ShortArray, n: Int) {
            val readIndex = (writeIndex - n).mod(capacity)
